...
func NewRunner(opts RunnerOptions) *Runner {
	...
	return &Runner{
		verbose: opts.Verbose || !IsInteractive(),
		output:  output,
	}
}
...
func newProgressModel(root TaskSpec, taskFunc func() error, cancels ...context.CancelFunc) progressModel {
	...
	s.Style = lipgloss.NewStyle().Foreground(lipgloss.Color(ColorMagenta))
	cancel := func() {}
...
func (m progressModel) View() tea.View {
	if m.cancelled {
		return tea.NewView(getStyle(ColorDarkRed).Render("● interrupted") + "\n")
	}
	if m.finished {
