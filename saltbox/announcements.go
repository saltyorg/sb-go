...
func LoadAllAnnouncementFiles() (*AnnouncementFile, *AnnouncementFile, error) {
	saltboxFile, err := LoadSingleAnnouncementFile(GetAnnouncementFilePath(layout.SaltboxRepoPath))
	if err != nil {
		return nil, nil, fmt.Errorf("error loading Saltbox announcements: %w", err)
	}

	sandboxFile, err := LoadSingleAnnouncementFile(GetAnnouncementFilePath(layout.Current().SandboxRepoPath))
	if err != nil {
...
func CheckForNewAnnouncementsAllRepos(saltboxBefore, saltboxAfter, sandboxBefore, sandboxAfter *AnnouncementFile) []*AnnouncementDiff {
	saltboxDiff := CheckSingleRepoAnnouncements("Saltbox", layout.SaltboxRepoPath, saltboxBefore, saltboxAfter)
	sandboxDiff := CheckSingleRepoAnnouncements("Sandbox", layout.Current().SandboxRepoPath, sandboxBefore, sandboxAfter)
...
func DisplayAnnouncements(runner *terminal.Runner, diffs []*AnnouncementDiff) error {
...
func ExecuteMigrations(ctx context.Context, runner *terminal.Runner, migrationRequests []MigrationRequest) error {
	...
	for _, migration := range migrationRequests {
		...
		case layout.Current().SaltboxModRepoPath:
			playbookPath = layout.SaltboxModPlaybookPath()
		default:
		...
		err := ansible.RunAnsiblePlaybook(ctx, migration.RepoPath, playbookPath, layout.AnsiblePlaybookBinaryPath, extraArgs, true)
		if err != nil {
