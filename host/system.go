...
func GetSaltboxUser() (string, error) {
	data, err := os.ReadFile(layout.SaltboxAccountsConfigPath)
	if err != nil {
	...
	for _, path := range paths {
		...
		if usage.fsid != "" {
			if existing, ok := seenFS[usage.fsid]; ok {
				terminal.Debug(verbosity, "Skipping %s (same filesystem as %s)", resolvedPath, existing)
				continue
			}
			seenFS[usage.fsid] = resolvedPath
		}

		terminal.Debug(verbosity, "Disk usage for %s: total=%s, available=%s, used=%.1f%%",
