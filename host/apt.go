...
func WaitForAptLock(ctx context.Context, verbose bool) error {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if !locked {
			if attempt == 1 {
				terminal.DebugBool(verbose, "Apt lock is available, proceeding...")
			} else {
				terminal.DebugBool(verbose, "Apt lock released, proceeding...")
			}
			return nil // Lock is available, proceed
		}

		// Lock is held, wait and retry
		terminal.DebugBool(verbose, "Waiting for apt lock to be released (attempt %d/%d), retrying in %v...",
