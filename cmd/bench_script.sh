...
speed_test() {
    local server_id="${1}"
    local nodeName="${2}"
    local logfile="${SB_BENCH_WORKDIR}/speedtest.log"

    if [ -z "${server_id}" ]; then
        "${SB_SPEEDTEST_BIN}" --progress=no --accept-license --accept-gdpr >"${logfile}" 2>&1
    else
        "${SB_SPEEDTEST_BIN}" --progress=no --server-id="${server_id}" --accept-license --accept-gdpr >"${logfile}" 2>&1
    fi
...
ipv4_info() {
    ...
    org="$(${ip_check_cmd} https://ipinfo.io/org)"
    city="$(${ip_check_cmd} https://ipinfo.io/city)"
    country="$(${ip_check_cmd} https://ipinfo.io/country)"
    region="$(${ip_check_cmd} https://ipinfo.io/region)"
    if [[ -n "${org}" ]]; then
...
install_speedtest() {
    if [[ -z "${SB_SPEEDTEST_BIN:-}" || ! -x "${SB_SPEEDTEST_BIN}" ]]; then
        _red "Error: Verified speedtest binary is unavailable.\n"
        return 1
    fi
...
ipv4_check=$( (ping -4 -c 1 -W 4 ipv4.google.com >/dev/null 2>&1 && echo true) || ${ip_check_cmd} -4 https://icanhazip.com 2>/dev/null)
ipv6_check=$( (ping -6 -c 1 -W 4 ipv6.google.com >/dev/null 2>&1 && echo true) || ${ip_check_cmd} -6 https://icanhazip.com 2>/dev/null)
