...
func TestGetGitCommitHash(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := WithExecutor(context.Background(), mock)

			// Call the function
			hash, err := GetGitCommitHash(ctx, tt.repoPath)
...
func TestGetGitCommitHash_NonexistentDirectory(t *testing.T) {
	...
	hash, err := GetGitCommitHash(WithExecutor(context.Background(), mock), nonexistentPath)
...
// TestWithExecutor tests invocation-scoped executor replacement.
func TestWithExecutor(t *testing.T) {
	mockExecutor := &MockCommandExecutor{}
	ctx := WithExecutor(context.Background(), mockExecutor)
	if got := executorFromContext(ctx); got != mockExecutor {
		t.Fatal("context executor was not used")
	}
}
...
func TestGetGitCommitHash_DirectoryNotExist(t *testing.T) {
	...
	hash, err := GetGitCommitHash(WithExecutor(context.Background(), mock), nonExistPath)
