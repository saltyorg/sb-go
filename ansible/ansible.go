...
import (
	...
	sbErrors "github.com/saltyorg/sb-go/signals"
...
func RunAnsiblePlaybook(ctx context.Context, repoPath, playbookPath, ansibleBinaryPath string, extraArgs []string, verbose bool) error {
	...
	if err != nil {
		...
		if sbErrors.HandleInterruptError(ctx, err) {
...
func PrepareAnsibleListTags(ctx context.Context, repoPath, playbookPath, extraSkipTags string, cache *Cache, verbosity int) ([]string, func(string) ([]string, error), string, error) {
	...
	if ok {
		terminal.Debug(verbosity, "PrepareAnsibleListTags: Cache found for %s", repoPath)
		if commit, commitOK := repoCache["commit"].(string); commitOK {
			...
			terminal.Debug(verbosity, "PrepareAnsibleListTags: Cached commit: %s, Current commit: %s", commit, currentCommit)

			if commit == currentCommit {
				// Cached tags are valid; create a parser function that returns them directly.
				cachedTags, tagsOk := repoCache["tags"].([]any)
				if tagsOk {
					terminal.Debug(verbosity, "PrepareAnsibleListTags: Using cached tags (%d tags)", len(cachedTags))
					...
					terminal.Debug(verbosity, "PrepareAnsibleListTags: Cached tags not in expected format")
				}
			} else {
				terminal.Debug(verbosity, "PrepareAnsibleListTags: Commit hash mismatch, will rebuild cache")
			}
		} else {
			terminal.Debug(verbosity, "PrepareAnsibleListTags: No valid commit in cache")
		}
	} else {
		terminal.Debug(verbosity, "PrepareAnsibleListTags: No cache found for %s", repoPath)
	}
...
// The context parameter allows for cancellation of the command execution.
// The verbosity parameter controls debug output (0 = no debug, >0 = debug).
func RunAndCacheAnsibleTags(ctx context.Context, repoPath, playbookPath, extraSkipTags string, cache *Cache, verbosity int) (bool, error) {
	...
	if args == nil && tagParser != nil {
		// Cached tags are available; retrieve and update the
		// cachedCommit already contains the current commit hash from PrepareAnsibleListTags.
		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Using cached tags for %s", repoPath)
		tags, err := tagParser("")
		if err != nil {
			return false, err
		}
		...
		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Cache updated with %d tags", len(tags))
		return false, nil // Cache was used, not rebuilt
	}

	if args != nil {
		// Use the executor interface to run the command
		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Running ansible-playbook for %s", repoPath)
		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Command: %s %v", layout.AnsiblePlaybookBinaryPath, args)
		output, err := executorFromContext(ctx).ExecuteContext(ctx, repoPath, layout.AnsiblePlaybookBinaryPath, args...)
		if err != nil {
			// Check if it's a user interrupt
			if sbErrors.HandleInterruptError(ctx, err) {
				return true, fmt.Errorf("command interrupted by user")
			}
			terminal.Debug(verbosity, "RunAndCacheAnsibleTags: ansible-playbook failed with error: %v", err)
			terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Command output:\n%s", string(output))
			return true, fmt.Errorf("ansible-playbook failed: %w\nOutput: %s", err, string(output))
		}

		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Raw ansible output length: %d bytes", len(output))
		terminal.Trace(verbosity, "RunAndCacheAnsibleTags: Raw output:\n%s", string(output))

		tags, err := tagParser(string(output))
		if err != nil {
			terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Failed to parse tags: %v", err)
			return true, err
		}

		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Parsed %d tags from ansible output", len(tags))

		currentCommit, err := git.GetGitCommitHash(ctx, repoPath)
		if err != nil {
			terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Failed to get git commit hash: %v", err)
			return true, err
		}

		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Current commit hash: %s", currentCommit)
		...
		terminal.Debug(verbosity, "RunAndCacheAnsibleTags: Cache rebuilt with %d tags, commit: %s", len(tags), currentCommit)
...
// The context parameter allows for cancellation of the command execution.
// The verbosity parameter controls debug output (0 = no debug, >0 = debug).
func RunAnsibleListTags(ctx context.Context, repoPath, playbookPath, extraSkipTags string, cache *Cache, verbosity int) ([]string, error) {
	...
	// Use the executor interface to run the command
	terminal.Debug(verbosity, "RunAnsibleListTags: Running ansible-playbook for %s", repoPath)
	terminal.Debug(verbosity, "RunAnsibleListTags: Command: %s %v", layout.AnsiblePlaybookBinaryPath, args)
	output, err := executorFromContext(ctx).ExecuteContext(ctx, repoPath, layout.AnsiblePlaybookBinaryPath, args...)
	if err != nil {
		// Check if it's a user interrupt
		if sbErrors.HandleInterruptError(ctx, err) {
		...
		terminal.Debug(verbosity, "RunAnsibleListTags: ansible-playbook failed with error: %v", err)
		terminal.Debug(verbosity, "RunAnsibleListTags: Command output:\n%s", string(output))
		return nil, fmt.Errorf("ansible-playbook failed: %w\nOutput: %s", err, string(output))
	}

	terminal.Debug(verbosity, "RunAnsibleListTags: Raw ansible output length: %d bytes", len(output))
	terminal.Trace(verbosity, "RunAnsibleListTags: Raw output:\n%s", string(output))

	tags, err := tagParser(string(output))
	if err != nil {
		terminal.Debug(verbosity, "RunAnsibleListTags: Failed to parse tags: %v", err)
		return nil, err
	}

	terminal.Debug(verbosity, "RunAnsibleListTags: Parsed %d tags from ansible output", len(tags))
