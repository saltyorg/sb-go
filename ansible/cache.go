...
// It sets up the cache with an empty data map and a fixed file path defined in layout.
...
func NewCache() (*Cache, error) {
	return NewCacheWithFile(layout.SaltboxCacheFile)
}
...
func (c *Cache) load() error {
	if _, err := os.Stat(c.file); os.IsNotExist(err) {
		return nil // File doesn't exist; start with an empty
	}
