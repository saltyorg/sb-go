...
func checkCachedTags(ctx context.Context, repoPath string, cache *Cache) ([]string, bool) {
...
func cacheTagsWithCommit(ctx context.Context, repoPath string, tags []string, cache *Cache) error {
	currentCommit, err := git.GetGitCommitHash(ctx, repoPath)
	if err != nil {
		return err
	}
...
func formatPlaybookError(ctx context.Context, playbookPath string, err error, stderrBuf *bytes.Buffer, verbose bool) error {
...
func isSaltboxModRepo(repoPath string) bool {
	return repoPath == layout.Current().SaltboxModRepoPath
}
