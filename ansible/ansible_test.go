...
func TestRunAnsiblePlaybook_ErrorHandling(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			binaryPath:   layout.AnsiblePlaybookBinaryPath,
...
func TestPrepareAnsibleListTags_CacheUsage(t *testing.T) {
	tmpDir := t.TempDir()
	cacheFile := filepath.Join(tmpDir, "test_json")
	...
	tests := []struct {
		...
		},
		{
			...
			repoPath:       layout.Current().SaltboxModRepoPath,
			setupCache:     false,
			matchingCommit: false,
			expectCmd:      true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.repoPath == layout.Current().SaltboxModRepoPath && !tt.expectCmd {
...
func TestSaltboxModSpecialCase(t *testing.T) {
	...
	cacheData := map[string]any{
		layout.Current().SaltboxModRepoPath: map[string]any{
			"commit": "abc123",
			"tags":   []any{"mod1", "mod2"},
		},
	}
