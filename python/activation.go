...
func managedReleaseLayout() releaseLayout {
	return releaseLayout{
		ansibleRoot:     layout.AnsibleVenvPath,
		ansibleReleases: layout.AnsibleReleasesPath,
		pythonReleases:  layout.PythonReleasesPath,
	}
}
...
func removePythonIfUnreferenced(path string) error {
	target, err := filepath.EvalSymlinks(filepath.Join(layout.AnsibleVenvPath, "venv"))
	if err == nil {
