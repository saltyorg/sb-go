...
// WithShareMode scopes sensitive-information masking to one MOTD invocation.
func WithShareMode(ctx context.Context, enabled bool) context.Context {
	return context.WithValue(ctx, shareModeContextKey{}, enabled)
}

func shareModeEnabled(ctx context.Context) bool {
	enabled, _ := ctx.Value(shareModeContextKey{}).(bool)
	return enabled
}
