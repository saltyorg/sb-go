...
func GenerateExampleConfig() (string, error) {
	schema, err := saltbox.LoadSchema(layout.SaltboxMOTDSchemaPath)
	if err != nil {
...
func generateSection(buf *strings.Builder, rule *saltbox.SchemaRule, indent string) {
...
func generateInstanceExample(buf *strings.Builder, itemRule *saltbox.SchemaRule, indent string) {
...
// It uses the description from schema but values from Go defaults.
func generateColorsSection(rule *saltbox.SchemaRule) string {
	var buf strings.Builder
